...
var load_file = function load_file(options, target, metadata, filename, cb) {
    fs.readFile(filename, {encoding: 'utf-8'}, function(err, content) {
        ...
        for (var lineIndex = 0; lineIndex < content.length; lineIndex++) {
            ...
            if (options.store_inflections) token.inflections = inflections;
            ...
            for (var inflectionIndex = 0; inflectionIndex < inflections.length; inflectionIndex++) {
               target[inflections[inflectionIndex]] = token; 
            }
            metadata.count += inflections.length;
        }

        cb(null, target, metadata);
    });
};
...
var SentiWS = function SentiWS(options, cb) {
    ...
    load_file(options, dict, null, file_positive, function(err, data, metadata) {
        if (err) return cb(err, null);

        load_file(options, dict, metadata, file_negative, function(err, data, metadata) {
            if (err) return cb(err, null);
            console.log(metadata);
            return cb(null, data, metadata);
        });
    });
}
